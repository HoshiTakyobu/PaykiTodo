...
- `1.13.19` re-audited recurring todo/event repository paths without finding a new data-layer recurrence bug, kept the existing canceled-tombstone and recurrence-anchor split behavior, limited expanded future recurring todo series to the first 30 rendered instances, pre-resolved dashboard todo group data for large-list scrolling, and made the Calendar timeline compute timed-event placement only for current page days with minute-level current-time refresh.
