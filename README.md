...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.6.52-debug.apk`
...
这个仓库当前已经整理到一个**已提交的 `1.6.52` 开发基线**。
...
- 内置 Wiki 目录可正常切换章节
- 设置页使用说明和提示音入口可直接跳转
- 电脑端日程时间轴点击日程卡片进入编辑/删除，展示色优先使用分组颜色，并移除顶部独立全天栏；日程卡片点击查找已改为兼容字符串 / 数字 ID，避免点击无响应
- 每日看板默认入口与看板视觉更新
- 每日看板待办区包含已错过但仍活动的待办
