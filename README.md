...
项目约定：命令行构建默认优先使用 Android Studio 自带的 `jbr`，优先顺序如下：

1. `G:\Android\Android Studio\jbr`
2. `%ProgramFiles%\Android\Android Studio\jbr`
3. `%LocalAppData%\Programs\Android Studio\jbr`

这样做是为了避免系统里默认的过新 Java 版本导致 Gradle / Kotlin DSL 构建失败。

只有在你明确想绕过这个约定时，才手动设置：

```powershell
$env:PAYKITODO_USE_SYSTEM_JAVA='1'
./gradlew.bat assembleDebug
```

结论：这个项目以后默认就按“Android Studio 里的 `jbr`”来构建，不再依赖记忆。
