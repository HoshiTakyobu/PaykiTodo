...
- Phone event creation supports `课程多时间段` for new events: multiple weekday + start/end slots share one event title, location, notes, reminder settings, color, countdown, and check-in configuration, then create one weekly recurring event series per slot without requiring a database schema change
