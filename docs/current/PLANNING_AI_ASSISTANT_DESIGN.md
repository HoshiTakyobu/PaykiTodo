...
- 本地优先：明确的 checkbox、`#ddl`、时间段、日期上下文等本地语法如果已经覆盖全部有效行，不调用 AI；这既节省 API，也避免模型重写已经清晰的日期和标题。显式语法中的解析错误也留在预览里让用户修正，不交给 AI 猜测吞掉。
- 日期上下文：AI prompt 会收到规划文档日期作为没有显式日期时的默认上下文，但行内显式日期必须优先，例如在 5 月 20 日文档中写 `5.29 【DDL】...` 仍应生成 5 月 29 日。
