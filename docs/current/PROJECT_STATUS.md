...
- Planning Desk heading context now treats headings containing `今日` / `今天` / `明天` as date context, so `# 今日计划` affects following undated schedule lines instead of being only explanatory copy.
