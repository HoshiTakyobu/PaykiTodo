...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.7.13-debug.apk`
...
- 快捷栏的“任务”会把当前行转为单个 checkbox，“子任务”会新起一行并缩进，避免同一行重复插入多个 `- [ ]`
- 识别后进入预览，用户勾选后再导入；预览内可先修改标题、分组、备注、DDL、日程起止时间、提醒输入和日程联动待办
- 导入成功后会自动给原文对应行追加 `#imported` 并保存当前规划文档，避免同一行被重复导入
- 规划台导入默认使用提前 5 分钟、全屏提醒、响铃 + 震动的强提醒策略
- 电脑端 Web 控制台新增 `规划台` Tab，支持同一份规划文档的编辑、保存、删除、识别、预览内编辑、导入、导入后 `#imported` 回写，并提供内置使用说明弹窗
...
这个仓库当前已经整理到一个**已构建的 `1.7.13` 开发基线**。
