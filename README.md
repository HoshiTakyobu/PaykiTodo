...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.7.7-debug.apk`
...
- 手机端提供纯色文档式 Markdown 原文编辑器、横向快捷输入栏和界面内说明页，支持任务、子任务、缩进、DDL、日程、提醒、分组、今天、明天等快捷输入
- 1.7.2 / 1.7.3 的手机端 Markdown 渲染组件已暂时回退，避免启动闪退；1.7.5 进一步修复规划台数据库迁移 schema 不一致导致的覆盖安装启动闪退；后续 Markdown 渲染会重新以更小范围实现
