...
- The project is currently at code version `1.7.22` / `versionCode 179`.
- Latest debug APK path after build: `app/build/outputs/apk/debug/PaykiTodo-1.7.22-debug.apk`.
- Latest build commands used Android Studio bundled JBR and succeeded:
  - `./gradlew.bat assembleDebug`
- This round adds phone-side AI call configuration and further improves Planning Desk writing space:
  - Settings now has `AI 调用配置` for provider name, Base URL, API Key, and model name.
  - The API Key is stored locally, intentionally excluded from backup JSON export, and preserved when importing backups that do not contain a key.
  - Planning Desk shortcut controls are collapsed by default and can be expanded from the toolbar or collapsed hint.
  - Planning Desk still uses local rule parsing by default; AI recognition remains a later preview-first implementation.
...
## Latest Changes In 1.7.22

1. Upgraded app version metadata to `1.7.22` / `versionCode 179`.
2. Settings -> AI 调用配置 saves provider name, Base URL, API Key, model name, and enabled state.
3. Backup export excludes the real AI API Key while preserving non-secret provider metadata; backup import does not clear an existing local key.
4. Planning Desk shortcut controls now default to collapsed and can be expanded on demand.
...
1. Install `PaykiTodo-1.7.22-debug.apk`.
...
15. Verify Planning Desk shortcuts are collapsed by default, can expand/collapse, and leave more writing space with the keyboard open.
16. Verify Settings -> AI 调用配置 saves and reloads provider name, Base URL, API Key, model name, and enabled state.
17. Verify backup JSON does not contain the real AI API Key.
18. Verify `任务M ddl 15:00` is recognized as a todo with today's 15:00 DDL.
19. Verify Planning Desk auto-save by typing, waiting at least 2 seconds, switching documents or leaving the page, then reopening.
20. Verify desktop web Planning Desk supports `Ctrl+S`, `Ctrl+Enter`, auto-save, and refuses import when no candidate is selected.
21. Verify `会议 9:00-10:00 讨论 #ddl 5.28` is recognized as a todo with DDL, not an event.
22. Verify basic navigation to 每日看板 / 我的任务 / 日历 / 设置 still works.
