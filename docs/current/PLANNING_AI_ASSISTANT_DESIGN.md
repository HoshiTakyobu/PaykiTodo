...
状态：`1.7.25` 已接入手机端和桌面 Web 规划台的真实网络调用；当前 `1.9.11` 继续沿用这条 AI 优先 / 本地规则回退链路，并保留 `1.8.5` 补充的单个 Provider“获取模型”“测试连接”和 Base URL endpoint 兼容修复。设置页保存多个 OpenAI-compatible Provider，规划台“识别”会在启用 AI 且配置完整时优先调用 AI，失败时回退本地规则；编辑、换行、自动保存和桌面导入不会隐式触发 AI。`1.9.9` 起进一步要求 AI 分组必须显式标记，且桌面端导入可以直接接收 AI 候选 ID；`1.9.11` 将 Provider 列表收口为摘要卡 + 更多菜单，避免窄屏配置页拥挤。
