...
- background capture recognition uses a `capture_processing` notification channel; completed capture notifications open the matching Planning Desk document after direct node insertion, while the phone Planning Desk `识别` button now starts background recognition instead of blocking the editor
...
- Planning Desk Outliner is partially user-facing in `1.12.2`: database version `20` adds `planning_nodes`, migrates existing Markdown lines into tree nodes, includes nodes in backup / restore, exposes repository / ViewModel CRUD plus Markdown import-export helpers, renders a phone outline editor by default, exposes desktop `/api/planning/nodes` routes, and adds a basic desktop Web node editor
- Planning Desk capture/share/photo/voice flows can directly write recognized candidates as planning nodes and linked official todos/events; parent-node completion now clears/reschedules reminders for all affected linked child items, and capture-created items disable reminder state when Android rejects scheduling
- Planning Desk Outliner still needs polish before being treated as complete: phone Enter/Tab/Shift+Tab/Backspace editing, long-press time/location editing, polished desktop reorder, and stricter migration/runtime QA remain pending
