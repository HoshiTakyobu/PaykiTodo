...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.7.9-debug.apk`
...
- 手机端提供纯色文档式 Markdown 原文编辑器、可选 Markdown 预览、横向快捷输入栏和界面内说明页，支持任务、子任务、缩进、DDL、日程、提醒、分组、今天、明天等快捷输入
...
- 本地规则解析器可识别 `- [ ]` 待办、子任务、日期上下文、常见 DDL 写法和轻量日程写法，例如 `明天 19:30-21:00 整理材料`
- 识别后进入预览，用户勾选后再导入；预览内可先修改标题、分组、备注、DDL、日程起止时间、提醒分钟和日程联动待办
- 导入成功后会自动给原文对应行追加 `#imported` 并保存当前规划文档，避免同一行被重复导入
- 规划台导入默认使用提前 5 分钟、全屏提醒、响铃 + 震动的强提醒策略
- 电脑端 Web 控制台新增 `规划台` Tab，支持同一份规划文档的编辑、保存、识别、预览内编辑、导入、导入后 `#imported` 回写，并提供内置使用说明弹窗
