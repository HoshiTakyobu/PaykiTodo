...
- Verify version `1.6.69` on device after desktop-event click-edit hardening
- Keep `README.md`, `CHANGELOG.md`, and current-state docs aligned with version `1.6.69`
...
- Desktop web todo cards still open preview first; desktop event cards now open the editor directly. Verify both flows on the actual desktop browser after installing `1.6.69`
...
- Calendar now shows lunar labels in timeline headers, month view, and agenda/list view; verify spacing and readability on device after installing `1.6.69`
- Desktop web existing event cards should open the event editor directly after installing `1.6.69` and refreshing the browser; this round could not reach `http://192.168.0.100:18765/` from the workstation because the request timed out
