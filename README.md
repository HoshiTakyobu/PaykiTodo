...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.6.49-debug.apk`
...
这个仓库当前已经整理到一个**已提交的 `1.6.49` 开发基线**。
...
- 每日看板日程卡片统一左侧竖线对齐，未开始日程增加同色细边框，正在进行日程只保留轻微内侧高光
...
- 提醒声音策略已扩展为“播放通道 + PaykiTodo 内部音量 + 可选临时系统通道提升 + 工作模式”，播放通道和日历默认项改为紧凑下拉，音量改为滑条与数值输入；工作模式保持静音 + 加强震动 + 全屏/无障碍兜底
