...
- Planning Desk Outliner has no known missing goal requirement after the `1.12.9` implementation pass, but still needs real-device and real-browser QA for launcher shortcuts, share targets, voice recognition, notifications, widget/launcher behavior, desktop Web node editing, same-level drag reorder, document switching, structure-heading migration, official-item delete/cancel detachment, and node time-field editing under real data
