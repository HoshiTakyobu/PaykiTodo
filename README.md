...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.7.22-debug.apk`
...
- 设置页新增 `AI 调用配置`，可以在手机端填写规划台后续 AI 识别要用的服务名称、Base URL、API Key 和模型名；真实 API Key 只保存在本机设置中，备份 JSON 不会导出
...
这个仓库当前已经整理到一个**已构建的 `1.7.22` 开发基线**。
