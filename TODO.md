...
- In-app Wiki should describe the current 1.10.0 settings, reminder, calendar, planning-desk, announcement, countdown-day, widget, focus-mode, desktop daily board, AI report archive, desktop sync, and input syntax accurately
...
- Countdown day should be device-tested after installing `1.10.0`: enabling 倒数日 in a DDL todo uses the todo DDL as target, enabling 倒数日 in an event uses event start time, expired targets disappear, no-DDL todos cannot become countdown targets, and edits preserve the switch state.
- No-DDL todos should be device-tested after installing `1.10.0`: new no-DDL todo from the normal editor and no-DDL todo imported from Planning Desk should both appear under 今日待办 on phone, Android widget, desktop daily board, and desktop paged todo management list, without enabling reminders or recurrence.
- Android desktop widget should be device-tested after installing `1.10.0`: at/after midnight, manual time/date change, timezone change, or app upgrade should refresh both the header date and list cards together; light-mode card text should remain readable over the board background, old focus-card content should not remain unless the launcher is still serving a stale widget instance that needs re-adding, the 今日看板 widget should include the 倒数日 section when targets exist, and the separate 倒数日 widget should show the nearest 3 targets.
- Settings -> AI 调用配置 should be device-tested after installing `1.10.0`: adding/editing/toggling/reordering/deleting a complete AI source should persist after leaving and reopening the page; incomplete enabled sources should show the required-field warning.
- Desktop sync should be device-tested after installing `1.10.0`: enabling sync and not authenticating from desktop for 5 minutes should auto-disable the setting and stop the foreground service; authenticating with the correct token within 5 minutes should keep it running, and saving a long Chinese Planning Desk document from desktop should not truncate the request body.
- Desktop web should be device/browser-tested after installing `1.10.0`: first connection should show `看板轻量数据`, the daily board should render immediately with countdown targets when present, clicking `加载待办管理列表` should request `/api/todos?offset=0&limit=80`, search should pass `q=...`, `加载更多` should append the next page, entering 日程时间轴 should load only the visible `/api/events?start=...&end=...` range, fast date switching should not show stale event data, and todo/event create/edit/delete should refresh without losing the current tab.
- AI 报告 should be device-tested after installing `1.10.0`: recent reports load first, type filter, keyword search, near-7/30/90-day range filters and `加载更多` work when enough reports exist, and report notifications can still open older reports by ID.
- Planning Desk announcements should be device-tested after installing `1.10.0`: ordinary board, Android widget, and desktop lightweight snapshot should still show active announcements after upgrading through `MIGRATION_14_15`, because announcement candidates now use the indexed `hasAnnouncementHint` field instead of scanning every planning note body.
...
- Calendar still needs real-device smoothness verification and profiling for drag/add/edit/delete under very large event sets; current `1.10.0` baseline reuses one date index across month/list/visible all-day surfaces, avoids loading the full active-event list on the daily board, uses a lightweight date window instead of allocating the whole long date list, subscribes phone calendar events by current visible date range instead of observing every active event, and keeps the Calendar top-bar title localized as `日历`.
- Desktop Web still needs real-browser verification with a large dataset; current `1.10.0` baseline keeps lightweight first board load, visible-range `/api/events?start=...&end=...`, paged/searchable `/api/todos`, indexed desktop todo paging, and daily-board countdown display while phone Calendar now also subscribes only to its visible event range.
