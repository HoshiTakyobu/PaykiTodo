...
- Phone Planning Desk Outliner `1.12.12` carries the `1.12.11` note-like interaction model: edit mode shows lightweight rows with a root active input line and child active input lines; editing an existing row and pressing Enter opens a focused same-level input at that row's position; preview mode shows per-row `⋯` actions; parent nodes with children are treated as structure headings and only leaf nodes sync into official todos/events
