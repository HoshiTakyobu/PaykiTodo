...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.7.24-debug.apk`
...
- 设置页 `AI 调用配置` 支持多个 AI 源，可新增、编辑、删除、启用/禁用并用上移/下移调整后续轮询优先级；真实 API Key 只保存在本机设置中，备份 JSON 不会导出
...
这个仓库当前已经整理到一个**已构建的 `1.7.24` 开发基线**。
