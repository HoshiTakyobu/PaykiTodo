...
- 当前调试 APK：`app/build/outputs/apk/debug/PaykiTodo-1.6.84-debug.apk`
...
- 识别后进入预览，用户勾选后再导入；日程默认可同时创建一条 DDL 等于日程结束时间的待办
- 规划台导入默认使用提前 5 分钟、全屏提醒、响铃 + 震动的强提醒策略
- 电脑端 Web 控制台新增 `规划台` Tab，支持同一份规划文档的编辑、保存、识别和导入
...
这个仓库当前已经整理到一个**已构建的 `1.6.84` 开发基线**。
