...
- Verify version `1.9.5` on device after adding AI 日报 / 周报, exact-alarm fallback, and the widget visual hotfix: scheduled daily report, manual daily report, weekly report, notification tap route, local fallback, and launcher widget rendering should all work
- Keep `README.md`, `CHANGELOG.md`, Wiki, in-app help sheets, desktop-web help, and current-state docs aligned with version `1.9.5`
...
- In-app Wiki should describe the current 1.9.5 settings, reminder, calendar, planning-desk, announcement, widget, focus-mode, AI report, and input syntax accurately
...
- Android desktop widget should be device-tested from the launcher widget picker after installing the current `1.9.5` debug APK: add widget, freely resize it, confirm the daily-board background/topbar/card visual direction, confirm the 4x5 default suggested size and static picker preview no longer look like a flat system list, show announcements at the top followed by greeting / focus summary / today todos / today schedules / tomorrow schedules, confirm row-level deep links, and confirm refresh after todo/event/planning-note changes.
