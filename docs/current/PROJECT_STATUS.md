...
- Latest debug APK output:
  - `app/build/outputs/apk/debug/PaykiTodo-1.7.6-debug.apk`
...
Current docs have been synchronized for `1.7.6`:
...
1. Device-side verification is required to confirm `1.7.6` still opens and the Planning Desk UI polish is acceptable`n2. Device-side verification is still required for the new reminder input UX, especially invalid-value red state and disabled save behavior
