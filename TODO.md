...
- Verify version `1.7.13` on device and desktop web after the Planning Desk parser boundary-fix pass
- Keep `README.md`, `CHANGELOG.md`, Wiki, in-app help sheets, desktop-web help, and current-state docs aligned with version `1.7.13`
...
- In-app Wiki should describe the current 1.7.13 settings, reminder, calendar, planning-desk, and input syntax accurately
...
- Desktop web timed and all-day event cards should open the editor directly by clicking the card; verify this in the actual desktop browser after installing `1.7.13`
...
- Desktop web todo cards still use preview first; event cards now open the editor directly. Verify event edit/save in the actual desktop browser after installing `1.7.13`
- Desktop web todo/event reminder input now accepts mixed reminder specs; verify valid examples and invalid late reminders on the actual desktop browser after installing `1.7.13`
- Calendar now shows lunar labels in timeline headers, month view, and agenda/list view; verify spacing and readability on device after installing `1.7.13`
- Desktop web existing event cards should open the editor directly after installing `1.7.13` and refreshing the browser; local Node DOM simulation with the live phone snapshot passed, but actual browser UI still needs device/browser verification after install
...
- Planning Desk should be device-tested for empty new documents, placeholder behavior, multi-document open/save/rename/delete, document search, shortcut-bar task/subtask input, mixed `#remind` parsing, the Planning Desk help sheet, Markdown preview checkbox toggling, editable recognition preview, import into todos/events, and automatic `#imported` marking
- Desktop web Planning Desk should be tested from a real browser connected to the phone after installing `1.7.13`, including the desktop Planning Desk help modal, document delete, editable preview, mixed reminder input, parser edge cases, and import marker write-back
