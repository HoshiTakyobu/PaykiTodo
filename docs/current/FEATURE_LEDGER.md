...
- Planning Desk Outliner data foundation is partially implemented in `1.12.1`: database version `20` adds `planning_nodes`, migrates existing Markdown lines into tree nodes, includes nodes in backup / restore, and exposes repository / ViewModel CRUD plus Markdown import-export helpers
- Planning Desk Outliner is not yet user-facing: the phone `PlanningDeskPanel` still renders the legacy Markdown editor, desktop sync has no `/api/planning/nodes` routes, desktop Web has no node tree editor, and capture flows still enter the preview/import pipeline instead of directly writing nodes
