...
- 设置页 `AI 调用配置` 支持多个 AI 源，可新增、编辑、删除、启用/禁用并用上移/下移调整轮询优先级；启用后规划台“识别”会优先调用 AI，把随手写的内容转成预览候选，失败或配置不完整时自动回到本地规则；真实 API Key 只保存在本机设置中，备份 JSON 不会导出
...
- 电脑端 Web 控制台新增 `规划台` Tab，支持同一份规划文档的编辑、自动保存、删除、识别、预览内编辑、导入、导入后 `#imported` 回写，并提供 `Ctrl+S` 保存、`Ctrl+Enter` 识别和内置使用说明弹窗
