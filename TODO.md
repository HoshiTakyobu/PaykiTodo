...
- Verify version `1.7.7` on device after adding the in-Planning-Desk help sheet
- Keep `README.md`, `CHANGELOG.md`, Wiki, and current-state docs aligned with version `1.7.7`
...
- In-app Wiki should describe the current 1.7.7 settings, reminder, calendar, planning-desk, and input syntax accurately
...
- Desktop web timed and all-day event cards should open the editor directly by clicking the card; verify this in the actual desktop browser after installing `1.7.7`
...
- Desktop web todo cards still use preview first; event cards now open the editor directly. Verify event edit/save in the actual desktop browser after installing `1.7.7`
...
- Calendar now shows lunar labels in timeline headers, month view, and agenda/list view; verify spacing and readability on device after installing `1.7.7`
- Desktop web existing event cards should open the editor directly after installing `1.7.7` and refreshing the browser; local Node DOM simulation with the live phone snapshot passed, but actual browser UI still needs device/browser verification after install
...
- Planning Desk should be device-tested for multi-document open/save/rename/delete, document search, shortcut-bar input, the new Planning Desk help sheet, editable recognition preview, import into todos/events, and automatic `#imported` marking
- Desktop web Planning Desk should be tested from a real browser connected to the phone after installing `1.7.7`, including editable preview and import marker write-back
