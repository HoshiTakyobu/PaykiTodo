...
- Phone Planning Desk Outliner `1.12.10` moves toward a note-like interaction model: edit mode shows lightweight rows with a root active input line and child active input lines, while preview mode shows per-row `⋯` actions; parent nodes with children are treated as structure headings and only leaf nodes sync into official todos/events
